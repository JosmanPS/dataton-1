createchart = function(filename, chart_no){
  ...
  d3.tsv(filename, type, function(error, data) {
    ...
    var chart = d3.select('#bar_bar_chart' + chart_no).append("svg")
...
createchart2 = function(filename, chart_no){
  ...
  d3.tsv(filename, type, function(error, data) {
    ...
    var x = d3.scale.linear().domain([0, d3.max(sortedData, barValue)]).range([0, maxBarWidth]);
    // svg container element
    var chart = d3.select('#bar_bar_chart' + String(chart_no)).append("svg")
      .attr("viewBox", "0 0 "+ (maxBarWidth + barLabelWidth + valueLabelWidth) + " "+ (gridLabelHeight + gridChartOffset + sortedData.length * barHeight))
      .attr("preserveAspectRatio", "xMinYMin meet")
...
createchart3 = function(filename, chart_no){
