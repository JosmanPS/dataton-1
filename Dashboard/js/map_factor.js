...
function draw_map(map_no, pre_fix_file_detailed, pre_fix_file_total, map_title, chartfunction, colors, color_scale) {
  ...
  var entran_total = (function () {
      ...
      $.ajax({
          'async': false,
          'global': false,
          'url': "Data/"+ pre_fix_file_total+".json",
          'dataType': "json",
          'success': function (data) {
              json = data;
          }
  ...
  function getColor(d) {
    ...
    return d > color_scale[7]  ? colors[7] :
           d > color_scale[6]  ? colors[6] :
           d > color_scale[5]  ? colors[5] :
           d > color_scale[4]  ? colors[4] :
           d > color_scale[3]  ? colors[3] :
           d > color_scale[2]  ? colors[2] :
           d > color_scale[1]  ? colors[1] :
                      colors[0];
  ...
  legend.onAdd = function (map) {
    ...
    for (var i = 0; i < color_scale.length; i++) {
      from = color_scale[i];
      to = color_scale[i + 1];
...
function draw_map_sentiment(map_no, pre_fix_file_detailed, pre_fix_file_total, map_title, chartfunction, colors, color_scale) {
  var h = $("#map-container" + String(map_no)).width();
  var offSet=8;
  $("#map" + String(map_no)).css("width", (h-offSet));
  var states = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15", "15"]
...
color_scale_tour = [0, .01, .02, .05, .1, .15, .2, .25]
color_scale_sent =  [0, .125, .25, .375, .50, .625, .75]
...
draw_map_sentiment("3","in_state_","sentimientos_turista","Sentimiento Turismo",createchart3,colors3, color_scale_sent);
draw_map_sentiment("4","in_state_","sentimientos_hogar","Sentimiento Pobladores",createchart3, colors3, color_scale_sent);
